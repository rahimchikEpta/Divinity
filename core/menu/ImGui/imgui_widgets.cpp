...
bool ImGui::ButtonEx ( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags ) {
	...
	// animations
	float deltatime = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.5f;
	// custom deltatime states for the circle animation
	float deltatimea = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.20f;
	float deltatimerad = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.15f;

	static std::map<ImGuiID, float> hover_animation;
	auto this_hovered = hover_animation.find ( id );
	if ( this_hovered == hover_animation.end () ) {
		hover_animation.insert ( { id, 0.f } );
		this_hovered = hover_animation.find ( id );
	}

	this_hovered->second = ImClamp ( this_hovered->second + ( 3.f * deltatime * ( hovered ? 1.f : -1.f ) ), 0.0f, 1.f );

	static std::map<ImGuiID, custom_gui::backend::button_state> faded_circle;
	auto this_faded_circle = faded_circle.find ( id );
	if ( this_faded_circle == faded_circle.end () ) {
		faded_circle.insert ( { id, {0.8f, 0.f, false, ImVec2 ( 0, 0 ), false} } );
		this_faded_circle = faded_circle.find ( id );
	}

	if ( pressed || this_faded_circle->second.active ) {
		if ( !this_faded_circle->second.pos_set ) {
			this_faded_circle->second.pos = ImGui::GetMousePos ();
			this_faded_circle->second.pos_set = true;
		}
		this_faded_circle->second.active = true;
		this_faded_circle->second.alpha = ImClamp ( this_faded_circle->second.alpha - ( 2.f * deltatimea ), 0.0f, 1.f );
		this_faded_circle->second.radius = ImClamp ( this_faded_circle->second.radius + ( 1.f * deltatimerad ), 0.0f, 1.f );

		if ( this_faded_circle->second.alpha <= 0.f )
			this_faded_circle->second.active = false;
	}
	else {
		this_faded_circle->second.alpha = 1.f;
		this_faded_circle->second.radius = 0.f;
		this_faded_circle->second.pos = ImVec2 ( 0, 0 );
		this_faded_circle->second.pos_set = false;
	}

	// Render

	float eased_circle = Easing::OutExpo ( this_faded_circle->second.radius );
	auto draw = window->DrawList;

	int alpha = ImGui::GetStyle ().Alpha * 255;
	int h_a = ImGui::GetStyle ().Alpha * ( this_hovered->second * 255 );
	int a = ImGui::GetStyle ().Alpha * ( this_faded_circle->second.alpha * 255 );

	ImColor cc = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], this_faded_circle->second.alpha );
	draw->AddRectFilled ( bb.Min, bb.Max, ImColor ( 12, 12, 12, alpha ), 2.f );
	draw->PushClipRect ( bb.Min, bb.Max );
	if ( !( flags & ImGuiButtonFlags_DontRenderCircle ) ) // Custom flag, meant for not rendering the circle. (for the keybinds LMAO)
		draw->AddCircleFilled ( this_faded_circle->second.pos, eased_circle * 230, cc, 360 );
	draw->PopClipRect ();
	// These are placed here for the circle animation as the clip rect does a solid non rounded square.
	draw->AddRect ( bb.Min, bb.Max, ImColor ( 12, 12, 12, alpha ) );
	draw->AddRect ( bb.Min, bb.Max, ImColor ( 30, 30, 32, alpha ), 2.f );
	draw->AddRect ( bb.Min, bb.Max, ImColor ( 60, 60, 60, h_a ), 2.f );
...
bool ImGui::ScrollbarEx ( const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v, float size_avail_v, float size_contents_v, ImDrawFlags flags ) {
	...
	ImColor cc_hide = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], 0.5f * ImGui::GetStyle().Alpha);
	ImColor cc_on = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], hov_t->second * ImGui::GetStyle().Alpha );
	window->DrawList->AddRectFilled ( grab_rect.Min, grab_rect.Max, cc_hide, 6.f );
	window->DrawList->AddRectFilled ( grab_rect.Min, grab_rect.Max, cc_on, 6.f );
...
bool ImGui::Checkbox ( const char* label, bool* v ) {
	...
	// animations
	float deltatime = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.8f;
	float deltatime2 = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.2f;
	static std::map<ImGuiID, float> hover_animation;
	auto this_hovered = hover_animation.find ( id );
	if ( this_hovered == hover_animation.end () ) {
		hover_animation.insert ( { id, 0.f } );
	...
	if ( this_toggled == press_anim.end () ) {
		...
		newstate.alpha = *v ? 1.f : 0.f;
		newstate.checked_state = *v ? true : false;
		press_anim.insert ( { id, newstate } );
		this_toggled = press_anim.find ( id );
	}
	
	static std::map<ImGuiID, float> not_toggled;
	auto this_not_toggled = not_toggled.find ( id );
	if ( this_not_toggled == not_toggled.end () ) {
		not_toggled.insert ( { id, 0.f } );
		this_not_toggled = not_toggled.find ( id );
	}

	this_toggled->second.alpha = ImClamp ( this_toggled->second.alpha + ( 3.f * deltatime * ( *v ? 1.f : -1.f ) ), 0.0f, 1.f );
	this_not_toggled->second = ImClamp ( this_not_toggled->second + ( 3.f * deltatime2 * ( !*v ? 1.f : -1.f ) ), 0.0f, 1.f );
	// Get the drawlist of the window aka the checkbox being rendered
	auto this_dl = window->DrawList;

	// Create an int value for the alpha of the toggled state
	float toggled_alpha = ImGui::GetStyle ().Alpha * ( this_toggled->second.alpha ); // You times the state using alpha so it fades out properly when opening and closing the menu.
	int h_a = ImGui::GetStyle ().Alpha * ( this_hovered->second * 255 );
	int a = ImGui::GetStyle().Alpha * 255;
	this_dl->AddRectFilled ( pos, ImVec2 ( pos.x + square_sz, pos.y + square_sz ), ImColor ( 12, 12, 12, a ), ImGui::GetStyle ().WindowRounding );
	this_dl->AddRect ( pos, ImVec2 ( pos.x + square_sz, pos.y + square_sz ), ImColor ( 26, 26, 26, a ), ImGui::GetStyle ().WindowRounding );
	this_dl->AddRect ( pos, ImVec2 ( pos.x + square_sz, pos.y + square_sz ), ImColor ( 45, 45, 45, h_a ), ImGui::GetStyle ().WindowRounding );
	this_dl->AddRectFilled ( pos, ImVec2 ( pos.x + square_sz, pos.y + square_sz ), ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], toggled_alpha ), ImGui::GetStyle ().WindowRounding );
	const char* text_display_end;
	text_display_end = FindRenderedTextEnd(label);
	this_dl->AddText(label_pos, ImColor(1.f, 1.f, 1.f, 0.5f * ImGui::GetStyle().Alpha), label, text_display_end);
	this_dl->AddText(label_pos, ImColor(1.f, 1.f, 1.f, toggled_alpha), label, text_display_end);
	// create the padding for the checkmark
	const float pad = ImMax ( 1.0f, ImFloor ( square_sz / 6.0f ) );
	float eased_not_toggled = *v == true ? Easing::InExpo ( this_not_toggled->second ) : Easing::OutSine ( this_not_toggled->second );
	this_dl->PushClipRect ( pos, pos + ImVec2 ( square_sz, square_sz ), true );
	RenderCheckMark ( ImGui::GetWindowDrawList(), check_bb.Min + ImVec2 (pad, pad + ( eased_not_toggled * square_sz )), ImColor (1.f, 1.f, 1.f, toggled_alpha), square_sz - pad * 2.0f);
	this_dl->PopClipRect ();
...
bool ImGui::BeginCombo ( const char* label, const char* preview_value, ImGuiComboFlags flags ) {
	...
	const ImRect bb ( ImVec2 ( window->DC.CursorPos.x, window->DC.CursorPos.y - 1 ), window->DC.CursorPos + ImVec2 ( w, label_size.y - 1 + style.FramePadding.y * 2.0f ) );
	...
	this_hovered->second = ImClamp ( this_hovered->second + ( 3.f * deltatime * ( hovered && !popup_open ? 1.f : -1.f ) ), 0.0f, 1.f );

	static std::map<ImGuiID, float> opened_animation;
	auto this_opened = opened_animation.find ( id );
	if ( this_opened == opened_animation.end () ) {
		opened_animation.insert ( { id, 0.f } );
		this_opened = opened_animation.find ( id );
	}

	this_opened->second = ImClamp ( this_opened->second + ( 3.f * deltatime * ( popup_open ? 1.f : -1.f ) ), 0.0f, 1.f );

	static std::map<ImGuiID, float> _notopened_animation;
	auto this_closed = _notopened_animation.find ( id );
	if ( this_closed == _notopened_animation.end () ) {
		_notopened_animation.insert ( { id, 0.f } );
		this_closed = _notopened_animation.find ( id );
	}

	this_closed->second = ImClamp ( this_closed->second + ( 3.f * deltatime * ( !popup_open ? 1.f : -1.f ) ), 0.0f, 1.f );
	// Render shape
	const ImU32 frame_col = GetColorU32 ( hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg );
	const float value_x2 = ImMax ( bb.Min.x, bb.Max.x - arrow_size );
	// RenderNavHighlight ( bb, id );
	// if ( !( flags & ImGuiComboFlags_NoPreview ) )
	int h = ImGui::GetStyle().Alpha * (this_hovered->second * 255);
	int opened_a = ImGui::GetStyle().Alpha * (this_opened->second * 255);
	int not_opened_a = ImGui::GetStyle().Alpha * (this_closed->second * 255);
	int alpha = ImGui::GetStyle().Alpha * 255;
	window->DrawList->AddRectFilled ( bb.Min, bb.Max, ImColor ( 12, 12, 12, alpha ), ImGui::GetStyle ().WindowRounding, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll );
	window->DrawList->AddRectFilled ( bb.Min, bb.Max, ImColor ( 20, 20, 20, opened_a ), ImGui::GetStyle ().WindowRounding, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll );
	window->DrawList->AddRect ( bb.Min, bb.Max, ImColor ( 25, 25, 25, not_opened_a ), ImGui::GetStyle ().WindowRounding, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll );
	//window->DrawList->AddRect ( bb.Min, bb.Max, ImColor ( 45, 45, 45, opened_a ), ImGui::GetStyle ().WindowRounding, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll );
	window->DrawList->AddRect ( bb.Min, bb.Max, ImColor ( 45, 45, 45, h ), ImGui::GetStyle ().WindowRounding, popup_open ? ImDrawFlags_RoundCornersTop : ImDrawFlags_RoundCornersAll );


	ImColor cc = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], (ImGui::GetStyle().Alpha * this_opened->second) );
	if ( ( flags & ImGuiComboFlags_RenderAsMultiSelect ) ) {
		ImGui::PushFont ( custom_gui::backend::multiselect_font );
		window->DrawList->AddText ( ImVec2 ( value_x2 - 1, bb.Max.y - ( ImGui::CalcTextSize ( "m" ).y ) - 2 ), ImColor ( 0.5f, 0.5f, 0.5f, ImGui::GetStyle().Alpha / 2 ), "m" );
		window->DrawList->AddText ( ImVec2 ( value_x2 - 1, bb.Max.y - ( ImGui::CalcTextSize ( "m" ).y ) - 2 ), ImColor ( 0.5f, 0.5f, 0.5f, (ImGui::GetStyle().Alpha * this_hovered->second )), "m" );
		window->DrawList->AddText ( ImVec2 ( value_x2 - 1, bb.Max.y - ( ImGui::CalcTextSize ( "m" ).y ) - 2 ), cc, "m" );
		ImGui::PopFont ();
		//window->DrawList->AddLine ( ImVec2 ( value_x2, bb.Min.y + 6 ), ImVec2 ( value_x2 + 15, bb.Min.y + 6 ), ImColor ( 125, 125, 125 ), 2.f );
		//window->DrawList->AddLine ( ImVec2 ( value_x2, bb.Min.y + 10 ), ImVec2 ( value_x2 + 15, bb.Min.y + 10 ), ImColor ( 125, 125, 125 ), 2.f );
		//window->DrawList->AddLine ( ImVec2 ( value_x2, bb.Min.y + 14 ), ImVec2 ( value_x2 + 15, bb.Min.y + 14 ), ImColor ( 125, 125, 125 ), 2.f );
	}
	else {
		ImGui::PushFont ( custom_gui::backend::select_fonts );
		window->DrawList->AddText ( ImVec2 ( value_x2, bb.Max.y - ( ImGui::CalcTextSize ( "d" ).y ) - 1 ), ImColor ( 0.5f, 0.5f, 0.5f, ImGui::GetStyle().Alpha / 2.f ), "d" );
		window->DrawList->AddText ( ImVec2 ( value_x2, bb.Max.y - ( ImGui::CalcTextSize ( "d" ).y ) - 1 ), ImColor ( 0.5f, 0.5f, 0.5f, (ImGui::GetStyle().Alpha * this_hovered->second) ), "d" );
		window->DrawList->AddText ( ImVec2 ( value_x2, bb.Max.y - ( ImGui::CalcTextSize ( "d" ).y ) - 1 ), cc, "d" );
		ImGui::PopFont ();
	}
...
float s_SliderCalcRatioFromValueT ( ImGuiDataType data_type, TYPE v, TYPE v_min, TYPE v_max, float power, float linear_zero_pos ) {
	if ( v_min == v_max )
		return 0.0f;
...
template<typename TYPE, typename SIGNEDTYPE, typename FLOATTYPE>
bool s_SliderBehaviorT ( const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent ) {
	ImGuiContext& g = *GImGui;
	const ImGuiStyle& style = g.Style;
	...
	*percent = floorf ( (float) ( *v - v_min ) / (float) ( v_max - v_min ) * 100.f );
	...
	// Output grab position so it can be displayed by the caller
	float grab_t = s_SliderCalcRatioFromValueT<TYPE, FLOATTYPE> ( data_type, *v, v_min, v_max, power, linear_zero_pos );
	if ( axis == ImGuiAxis_Y )
		grab_t = 1.0f - grab_t;
	const float grab_pos = ImLerp ( slider_usable_pos_min, slider_usable_pos_max, grab_t );
	if ( axis == ImGuiAxis_X )
...
bool s_SliderBehavior ( const ImRect& bb, ImGuiID id, ImGuiDataType data_type, void* v, const void* v_min, const void* v_max, const char* format, float power, ImGuiSliderFlags flags, ImRect* out_grab_bb, int* percent ) {
	switch ( data_type ) {
		case ImGuiDataType_S32:
			IM_ASSERT ( *(const ImS32*) v_min >= IM_S32_MIN / 2 && *(const ImS32*) v_max <= IM_S32_MAX / 2 );
			return s_SliderBehaviorT<ImS32, ImS32, float > ( bb, id, data_type, (ImS32*) v, *(const ImS32*) v_min, *(const ImS32*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_U32:
			IM_ASSERT ( *(const ImU32*) v_min <= IM_U32_MAX / 2 );
			return s_SliderBehaviorT<ImU32, ImS32, float > ( bb, id, data_type, (ImU32*) v, *(const ImU32*) v_min, *(const ImU32*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_S64:
			IM_ASSERT ( *(const ImS64*) v_min >= IM_S64_MIN / 2 && *(const ImS64*) v_max <= IM_S64_MAX / 2 );
			return s_SliderBehaviorT<ImS64, ImS64, double> ( bb, id, data_type, (ImS64*) v, *(const ImS64*) v_min, *(const ImS64*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_U64:
			IM_ASSERT ( *(const ImU64*) v_min <= IM_U64_MAX / 2 );
			return s_SliderBehaviorT<ImU64, ImS64, double> ( bb, id, data_type, (ImU64*) v, *(const ImU64*) v_min, *(const ImU64*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_Float:
			IM_ASSERT ( *(const float*) v_min >= -FLT_MAX / 2.0f && *(const float*) v_max <= FLT_MAX / 2.0f );
			return s_SliderBehaviorT<float, float, float > ( bb, id, data_type, (float*) v, *(const float*) v_min, *(const float*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_Double:
			IM_ASSERT ( *(const double*) v_min >= -DBL_MAX / 2.0f && *(const double*) v_max <= DBL_MAX / 2.0f );
			return s_SliderBehaviorT<double, double, double> ( bb, id, data_type, (double*) v, *(const double*) v_min, *(const double*) v_max, format, power, flags, out_grab_bb, percent );
		case ImGuiDataType_COUNT: break;
	}
	IM_ASSERT ( 0 );
	return false;
...
bool ImGui::SliderScalar ( const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags ) {
	...
	// animations
	float deltatime = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.3f;

	// g.ActiveId == id
	static std::map<ImGuiID, float> circle_out;
	auto this_out = circle_out.find ( id );
	if ( this_out == circle_out.end () ) {
		circle_out.insert ( { id, 0.f } );
		this_out = circle_out.find ( id );
	}

	this_out->second = ImClamp ( this_out->second + ( 3.f * deltatime * ( hovered && g.ActiveId != id ? 1.f : -1.f ) ), 0.0f, 1.f );

	static std::map<ImGuiID, float> circle_held;
	auto held_out = circle_held.find ( id );
	if ( held_out == circle_held.end () ) {
		circle_held.insert ( { id, 0.f } );
		held_out = circle_held.find ( id );
	}

	held_out->second = ImClamp ( held_out->second + ( 3.f * deltatime * ( g.ActiveId == id ? 1.f : -1.f ) ), 0.0f, 1.f );
	int int_alpha = ImGui::GetStyle().Alpha * 255;
	float float_alpha = ImGui::GetStyle().Alpha;
	// Draw frame
	auto this_dl = window->DrawList;
	this_dl->AddRectFilled ( frame_bb.Min, frame_bb.Max, ImColor ( 12, 12, 12, int_alpha ), ImGui::GetStyle ().WindowRounding );
	this_dl->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 25, 25, 25, int_alpha ), ImGui::GetStyle ().WindowRounding );
	...
	// Slider behavior
	ImRect grab_bb;
	const bool value_changed = s_SliderBehavior ( frame_bb, id, data_type, p_data, p_min, p_max, format, 1.f, ImGuiSliderFlags_None, &grab_bb, &percent );
	if ( value_changed )
		MarkItemEdited ( id );
	ImRect slider_bb = ImRect ( frame_bb.Min, frame_bb.Max );
	...
	static bool actively_held = g.ActiveId == id;
	char value_buf[ 64 ];
	const char* value_buf_end = value_buf + DataTypeFormatString ( value_buf, IM_ARRAYSIZE ( value_buf ), data_type, p_data, format );

	float hover_eased = Easing::OutQuad ( this_out->second );
	float held_eased = Easing::OutQuad ( held_out->second );

	float fheld_out = held_out->second * ImGui::GetStyle().Alpha;
	float fthis_out = this_out->second * ImGui::GetStyle().Alpha;
	ImVec2 text_size = ImGui::CalcTextSize ( value_buf, value_buf_end );
	ImColor cc = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], float_alpha );
	ImColor cc_2 = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], float_alpha / 2.f );
	this_dl->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 25, 25, 25, int_alpha ), ImGui::GetStyle ().WindowRounding );
	this_dl->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 45 / 255.f, 45 / 255.f, 45 / 255.f, g.ActiveId == id ? fheld_out : fthis_out ), ImGui::GetStyle ().WindowRounding );
	if (it_old_value->second > 1)
		this_dl->AddRectFilled ( slider_bb.Min, ImVec2(slider_bb.Min.x + it_old_value->second, slider_bb.Max.y), cc, ImGui::GetStyle().WindowRounding);
	//this_dl->AddLine ( ImVec2 ( slider_bb.Min.x - 1, slider_bb.Min.y - 1 ), ImVec2 ( slider_bb.Min.x - 1, slider_bb.Max.y ), ImColor ( 12, 12, 12, int_alpha ), 2.f );
	ImGui::GetForegroundDrawList ()->AddRectFilled ( ImVec2 ( slider_bb.Min.x - ( text_size.x / 2 ) + it_old_value->second, slider_bb.Min.y - 5 - text_size.y - 2 ), ImVec2 ( slider_bb.Min.x + ( text_size.x / 2 + 2 ) + it_old_value->second, slider_bb.Min.y - 5 ), ImColor ( 15 / 255.f, 15 / 255.f, 15 / 255.f, g.ActiveId == id ? fheld_out : fthis_out ), 2.f );
	ImGui::GetForegroundDrawList ()->AddRect ( ImVec2 ( slider_bb.Min.x - ( text_size.x / 2 ) + it_old_value->second, slider_bb.Min.y - 5 - text_size.y - 2 ), ImVec2 ( slider_bb.Min.x + ( text_size.x / 2 + 2 ) + it_old_value->second, slider_bb.Min.y - 5 ), ImColor ( 45 / 255.f, 45 / 255.f, 45 / 255.f, g.ActiveId == id ? fheld_out : fthis_out ), 2.f );
	ImGui::GetForegroundDrawList ()->AddText ( ImVec2 ( slider_bb.Min.x - ( text_size.x / 2 - 2 ) + it_old_value->second, slider_bb.Min.y - 5 - text_size.y ), ImColor ( 200 / 255.f, 200 / 255.f, 200 / 255.f, g.ActiveId == id ? fheld_out : fthis_out ), value_buf, value_buf_end );

	//this_dl->AddText ( ImVec2 ( frame_bb.Max.x - text_size.x - 1, frame_bb.Max.y - text_size.y - 12 ), ImColor ( 255, 255, 255 ), value_buf, value_buf_end );
	// Display value using user-provided display format so user can add prefix/suffix/decorations to the value.
	...
	//  RenderTextClipped ( frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2 ( 0.5f, 0.5f ) );
	  ...
	  // making the string invisible esentially
	std::string blanked_label = std::string ( "##" ).append ( label );
	if ( label_size.x > 0.0f )
		RenderText ( ImVec2 ( frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y ), blanked_label.c_str () );

	ImGui::GetStyle ().FramePadding = ImVec2 ( 3, 3 );
	return value_changed;
}
...
bool ImGui::InputTextEx ( const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data ) {
	...
	bool toggled = g.ActiveId == id;
	float deltatime = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime;
	// Render frame
	if ( !is_multiline ) {
		RenderNavHighlight ( frame_bb, id );
		static std::map<ImGuiID, float> hovered_anim;
		...
		if ( hovered || toggled )
			...
			hov_t->second = ImMax ( 0.f, hov_t->second - 1.f * deltatime );
		int alpha = ImGui::GetStyle().Alpha * 255;
		int a = ImGui::GetStyle().Alpha * (hov_t->second * 255);
		window->DrawList->AddRectFilled ( frame_bb.Min, frame_bb.Max, ImColor ( 12, 12, 12, alpha ), ImGui::GetStyle ().WindowRounding );
		window->DrawList->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 25, 25, 25, alpha ), ImGui::GetStyle ().WindowRounding );
		window->DrawList->AddRect ( frame_bb.Min, frame_bb.Max, ImColor ( 45, 45, 45, a ), ImGui::GetStyle ().WindowRounding );
	}
	...
	if ( render_cursor || render_selection ) {
		...
		if ( render_cursor ) {
			...
			float deltatime = ( 1.0f / 0.2f ) * ImGui::GetIO ().DeltaTime * 0.3f;
			static std::map<ImGuiID, float> blink_animation;
			auto blink = blink_animation.find ( id );
			if ( blink == blink_animation.end () ) {
				blink_animation.insert ( { id, 0.f } );
				blink = blink_animation.find ( id );
			}

			blink->second = ImClamp ( blink->second + ( 3.f * deltatime * ( cursor_is_visible ? 1.f : -1.f ) ), 0.0f, 1.f );
			float eased_alpha = cursor_is_visible ? Easing::InExpo ( blink->second ) : Easing::OutExpo ( blink->second );
			ImColor cc = ImColor ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], eased_alpha );
			if ( cursor_screen_rect.Overlaps ( clip_rect ) ) {

				draw_window->DrawList->AddLine ( cursor_screen_rect.Min, cursor_screen_rect.GetBL (), cc, 2.f );
			}
...
bool ImGui::ColorPicker4 ( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col ) {
	...
	if ( ( flags & ImGuiColorEditFlags_NoInputs ) == 0 ) {
		...
		if ( ImGui::Button ( "Copy", ImVec2 ( ImGui::GetWindowSize ().x / 2 - 7, 20 ) ) ) {
			ImGui::custom::col[ 0 ] = col[ 0 ];
			ImGui::custom::col[ 1 ] = col[ 1 ];
			ImGui::custom::col[ 2 ] = col[ 2 ];
			ImGui::custom::col[ 3 ] = col[ 3 ];
		}
		ImGui::SameLine ();
		if ( ImGui::Button ( "Paste", ImVec2 ( ImGui::GetWindowSize ().x / 2 - 7, 20 ) ) ) {
			col[ 0 ] = ImGui::custom::col[ 0 ];
			col[ 1 ] = ImGui::custom::col[ 1 ];
			col[ 2 ] = ImGui::custom::col[ 2 ];
			col[ 3 ] = ImGui::custom::col[ 3 ];
		}
		ImGui::Spacing ();
		// PopItemWidth();
	}
...
bool ImGui::Selectable ( const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg ) {
	...
	const ImVec4 text_act = ImVec4 ( c::menu_accent[ 0 ], c::menu_accent[ 1 ], c::menu_accent[ 2 ], 1.f );
	const ImVec4 text_hov = ImVec4 ( 200 / 255.f, 200 / 255.f, 200 / 255.f, 1.f );
	const ImVec4 text_dis = ImVec4 ( 125 / 255.f, 125 / 255.f, 125 / 255.f, 1.f );
