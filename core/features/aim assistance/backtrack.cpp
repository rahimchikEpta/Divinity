...
void lag_compensation::update ( int stage ) {
	if ( !settings::aimbot::backtrack_enable || !csgo::local_player || !csgo::local_player->is_alive () ) {
	...
	if ( stage == FRAME_RENDER_START ) {
		for ( int i = 1; i <= 64; i++ ) {
			...
			while ( settings::aimbot::backtrack_ping_spike ? ( records[ i ].size () > 3 && records[ i ].size () > static_cast<size_t>( time_to_ticks ( 0.2f ) + nci->get_latency ( 0 ) ) ) : ( records[ i ].size () > 3 && records[ i ].size () > static_cast<size_t>( time_to_ticks ( 0.2f ) ) ) )
				records[ i ].pop_back ();
...
void lag_compensation::run ( c_usercmd* cmd ) {
	if ( !settings::aimbot::backtrack_enable )
		return;
	...
	for ( int i = 1; i <= 64; i++ ) {
		...
		auto angle = math::calculate_angle2 ( local_eye_pos, origin, cmd->viewangles );
	...
	if ( best_target ) {
		...
		for ( size_t i = 0; i < records[ best_target_index ].size (); i++ ) {
			...
			auto angle = math::calculate_angle2 ( local_eye_pos, record.origin, cmd->viewangles );
			auto fov = std::hypotf ( angle.x, angle.y );
			if ( fov < best_fov ) {
				best_fov = fov;
				best_record = i;
...
void lag_compensation::update_incoming_sequences ( i_net_channel_t* net_channel ) {
...
void lag_compensation::add_latency_to_net_channel ( i_net_channel_t* net_channel, float latency ) {
