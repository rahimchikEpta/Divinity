...
void rendering::initialize() {
	rendering::fonts::watermark = interfaces::surface->font_create();
	rendering::fonts::name = interfaces::surface->font_create();
	rendering::fonts::esp = interfaces::surface->font_create();

	interfaces::surface->set_font_glyph(rendering::fonts::watermark, ("Lucida Console"), 10, 0, 0, 0, font_flags::fontflag_dropshadow);
	interfaces::surface->set_font_glyph(rendering::fonts::name, ("Small Fonts"), 8, 400, 0, 0, font_flags::fontflag_dropshadow | font_flags::fontflag_antialias);
	interfaces::surface->set_font_glyph(rendering::fonts::esp, ("Tahoma"), 12, 700, 0, 0, font_flags::fontflag_dropshadow);
}

void rendering::cornered_rect(int x, int y, int w, int h, color colour) {
...
void rendering::line(int x1, int y1, int x2, int y2, color colour) {
	interfaces::surface->set_drawing_color(colour.r, colour.g, colour.b, colour.a);
	interfaces::surface->draw_line(x1, y1, x2, y2);
}

void rendering::text(int x, int y, unsigned long font, std::string string, bool text_centered, color colour) {
...
void rendering::rect(int x, int y, int w, int h, color color) {
	interfaces::surface->set_drawing_color(color.r, color.g, color.b, color.a);
	interfaces::surface->draw_outlined_rect(x, y, w, h);
}

void rendering::filled_rect(int x, int y, int w, int h, color colour) {
	interfaces::surface->set_drawing_color(colour.r, colour.g, colour.b, colour.a);
	interfaces::surface->draw_filled_rectangle(x, y, w, h);
}

void rendering::textured_polygon(int n, vertex_t* vertice, color col) {
...
void rendering::circle(int x, int y, int r, int s, color col) {
...
vec2_t rendering::get_text_size(unsigned long font, std::string text) {
